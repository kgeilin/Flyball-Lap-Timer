...
//pinMode(buzzer, OUTPUT);
  ...
  //lcd.noDisplay();
 lcd.setCursor(0, 3); // set cursor to [character-place 0, line 0]. (first row)
 lcd.print("READY");
...
void loop()
{
 ...
 if (digitalRead(Start) == LOW || digitalRead(startButton) == LOW )   {
     ...
     lcd.display();
     delay(1500); // for debounce and stop ghost trigger
     }
...
if (digitalRead(Stop) == LOW || digitalRead(stopButton) == LOW )   {
     ...
     delay(1500); // for debounce stop ghost trigger
     digitalWrite(readyLED, HIGH);
     //oldTime = 
     }
...
//ones' place (most sig millisecond digit )
 ...
 //tenths' place (2nd sig millisecond digit )
